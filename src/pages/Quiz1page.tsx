...
const Quiz1page = () => {
  ...
  if (currQuestion > QUIZ_QUESTIONS.length - 1)
    return (
      <>
        <div className="w-full max-w-[90%] sm:max-w-[70%] flex flex-col justify-center items-center h-screen bg-cover bg-center gap-4 sm:gap-2 md:gap-6">
          {`YOU´RE FINISH. CONGRATULATIONS! ${points} points`}{" "}
  ...
  const Option = ({
    ...
    return (
      <div
        className="w-full max-w-[90%] sm:max-w-[70%] bg-gray-300 text-black p-2 rounded-lg font-[Montserrat] text-left pb-[12px] pl-[21px] text-[clamp(14px, 2vw, 16px)]"
        onClick={() => handleOnClick(isAnswer)}
      >
        {`${abc}. ${optionString}`}
      </div>
    );
  ...
  for (let i = 0; i < options.length; i++) {
    while (true) {
      ...
      if (i === 0) {
        newOptions[num] = { string: options[i], isAnswer: true };
        break;
      }
      if (!newOptions[num]) {
        newOptions[num] = { string: options[i], isAnswer: false };
        break;
      }
    }
  }
  ...
  return (
    ...
    >
      ...
      <div className="w-full max-w-[90%] sm:max-w-[70%] text-[20px] text-right pr-8 mb-48 mt-32">
        X
      </div>

      <div className="w-full max-w-[90%] sm:max-w-[70%] border border-black h-[6px] -mb-4 bg-white">
        <br></br>
      </div>
      ...
      {newOptions.map((option, index) => {
        // Map over the newOptions array and generate the JSX
